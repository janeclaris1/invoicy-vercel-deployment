...
import { ArrowLeft, Mail, Phone, Building2, Loader2, Plus, PhoneCall, Mail as MailIcon, Calendar, FileText, CheckSquare, Target } from "lucide-react";
...
const LeadDetailPage = () => {
  ...
  const fetchLead = async () => {
    if (!id) return;
    try {
      const res = await axiosInstance.get(API_PATHS.CRM.LEAD(id));
      setLead(res.data);
    } catch (err) {
  ...
  const fetchActivities = async () => {
    if (!id) return;
    try {
      const res = await axiosInstance.get(API_PATHS.CRM.ACTIVITIES, { params: { leadId: id } });
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        <Link to="/crm/leads" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            {lead.score != null && <span className="ml-2 text-sm text-gray-500">Score: {lead.score}</span>}
            {lead.email && (
              <div className="flex items-center gap-2 text-sm text-gray-600 mt-3 mb-2">
            ...
            {lead.source && <p className="text-sm text-gray-500 mt-2">Source: {lead.source}</p>}
            {lead.notes && (
            ...
            <div className="relative py-2">
              ...
              {activities.length === 0 ? (
                <div className="p-8 text-center text-gray-500 text-sm">
                  No activities yet. Log a call, email, meeting, or note to build this lead’s timeline.
                </div>
              ) : (
